<?php
   ...
   ?>
					...
					<form class="form" id="sendForm">
						<div class="card-body">
							<div class="form-group row ">
								...
								<div class="col-lg-9">
									<select class="form-control is-valid" name="from" id="from">
									    <option value="0"> - Chọn Momo - </option>
										<?php 
                                            $getdl = $db->fetch_assoc("SELECT * FROM `cron_momo` WHERE `user_id` = '".$accounts['username']."' AND `status` = 'success' ",0);
							...
							<div class="form-group row ">
								...
								<?php
                                   $tmpDiscount = json_decode(file_get_contents($_DOMAIN."api/momo/send_bank/list"),true);
                                   $bankname = array();
                                   $codebank = array();
                                   $shortbank = array();
	...
	<!--begin::Page Scripts(used by this page)-->
		<script src="/assets/js/pages/crud/ktdatatable/base/send_bank.js?v=<?=rand()?>"></script>
	<!--end::Page Scripts-->
	<script>
	...
	$(document).ready(function() {
		$("#account_number , #from , #bankcode").change(function() {
			...
			if(account_number && bankcode) {
				$.ajax({
					url: "/api/momo/send_bank/checkname",
					method: "post",
					data: {
						from: from,
